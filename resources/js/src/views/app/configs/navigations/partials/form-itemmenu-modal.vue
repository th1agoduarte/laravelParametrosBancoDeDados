    ...
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header" id="formParameterModalLabel">
          <h4 class="modal-title">Menu: {{props.menu.name}} - Main Menu Item</h4>
          <button
            type="button"
            data-dismiss="modal"
        ...
        <div class="modal-footer justify-content-center">
          <div class="forgot login-footer">
            <span>This Menu Item will be added to the Main Menu</span>
          </div>
        </div>
      </div>
    </div>
...
onMounted(() => {
  ...
  if (props.typeModeUpdate === "edit_item") {
    ...
    data.menu_id = props.dataItem.menu_id;
    data.route = props.dataItem.route;
  }else{
    data.menu_id = props.menu.id_update;
  }
...
const addHeader = () => {
  ...
  data.form.menu_id = data.menu_id;
  data.form.route = data.route;
  data.form.id = data.id;
  if (props.typeModeUpdate === "edit_item") {
    data.form.post(route("app.navigations.itemmenu.update"), {
      preserveState: true,
      preserveScroll: true,
      onSuccess: (page) => {
        new window.Swal({
  ...
  if (props.typeModeUpdate === "add_item") {
    data.form.post(route("app.navigations.itemmenu.store"), {
      preserveState: true,
      preserveScroll: true,
      onSuccess: (page) => {
        new window.Swal({
