      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="input-group">
            <input
              ...
              placeholder="Nome do Menu Principal"
            />
          </div>
          <div v-if="data.erros" class="text-danger">{{ data.erros.name }}</div>

          <label for="descripts" class="form-label mt-3">Icon Font Awesome (fa-solid fa-house)</label>
          <div class="input-group">
            <input
              v-model="data.icon"
              type="text"
          ...
          <label for="route" class="form-label mt-3">Route</label>
          <div class="input-group">
        ...
        <div class="modal-footer justify-content-center">
          <div class="forgot login-footer">
            <span>This menu will be added to the Main Menu</span>
          </div>
        </div>
      </div>
    </div>
...
const addHeader = () => {
  ...
  if (data.icon == "")
    Object.assign(data.erros, {
      icon: "The Icon field is mandatory.",
    });
  ...
  data.form.icon = data.icon;
  data.form.route = data.route;
  data.form.id = data.id;

  if (props.typeModeUpdate === "edit") {
    data.form.post(route("app.navigations.update"), {
      preserveState: true,
      preserveScroll: true,
      onSuccess: (page) => {
        new window.Swal({
  ...
  if (props.typeModeUpdate === "add") {
    data.form.post(route("app.navigations.store"), {
      preserveState: true,
      preserveScroll: true,
      onSuccess: (page) => {
        new window.Swal({
